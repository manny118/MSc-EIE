...
# declare some arrays to store the computed values
r2_preamp_in = []
amp1_in = []
amp2_in = []
preamp_in = []
...
for i in range(0, len(optical_signals)):
  for k, v in list(optical_signals[i].loc_in_to_state.items()):
    if str(k) == "<r1-r2-amp1 11.0dB>":  # amp1's input
      amp1_in.append(wattsTodBm(v['power']))
    elif str(k) == "<r1-r2-amp2 11.0dB>":  # amp2's input
      amp2_in.append(wattsTodBm(v['power']))
    elif str(k) == "<r2-preamp 11.0dB>":  # input of R2's pre-amp
      preamp_in.append(wattsTodBm(v['power']))


# compute the end-to-end tilt for each input
amp1_in_dBm_tilt = computeEndToEndTilt(amp1_in)
amp2_in_dBm_tilt = computeEndToEndTilt(amp2_in)
preamp_in_dBm_tilt = computeEndToEndTilt(preamp_in)

# round the values to 2 decimal places
amp1_in_dBm_tilt = round(amp1_in_dBm_tilt, 2)
amp2_in_dBm_tilt = round(amp2_in_dBm_tilt, 2)
preamp_in_dBm_tilt = round(preamp_in_dBm_tilt, 2)

# combine the results for each input
actual_result = [amp1_in_dBm_tilt, amp2_in_dBm_tilt, preamp_in_dBm_tilt]

print("amp1_in_dBm_tilt: ", amp1_in_dBm_tilt)
print("amp2_in_dBm_tilt: ", amp2_in_dBm_tilt)
...
class TestSRS(unittest.TestCase):
  ...
  def test_SRS(self):
    expected_result = [0.62, 1.33, 2.15]
